function Game(assets) {
  ...
  this.showEnemyDirection = function() {
    if (that.botArray.length > 0) {
      for (var i = 0; i < that.botArray.length; i++) {
        var dx =
          that.actor.property.canvasX - that.botArray[i].botProperty.canvasX;
        var dy =
          that.actor.property.canvasY - that.botArray[i].botProperty.canvasY;
        if (dx <= 700) {
          ...
          that.ctx.translate(
            that.actor.property.canvasX,
            that.actor.property.canvasY
          );
      ...
      for (var i = 0; i < that.bulletArray.length; i++) {
        ...
        if (that.bulletArray[i].actor.character === 'actor') {
          for (var j = 0; j < that.botArray.length; j++) {
            if (
              that.bulletArray[i].fX >= that.botArray[j].botProperty.canvasX &&
              that.bulletArray[i].fX <=
                that.botArray[j].botProperty.canvasX +
                  that.botArray[j].botProperty.characterWidth &&
              that.bulletArray[i].fY >= that.botArray[j].botProperty.canvasY &&
              that.bulletArray[i].fY <=
                that.botArray[j].botProperty.canvasY +
                  that.botArray[j].botProperty.characterHeight
            ) {
          ...
          if (
            that.bulletArray[i].fX >= that.actor.property.canvasX &&
            that.bulletArray[i].fY >= that.actor.property.canvasY &&
            that.bulletArray[i].fY <=
              that.actor.property.canvasY +
                that.actor.property.canvasY +
                that.actor.property.characterHeight
          ) {
            that.bulletArray.splice(i, 1);
  ...
  this.dropGun = function() {
    if (that.weaponDropCounter % 1000 == 0) {
      that.weaponArray.push(
        new Weapon(that.ctx, that.detectCollision, that.assets)
      );
    }
  ...
  this.findGun = function() {
    if (that.weaponArray.length > 0) {
      for (var i = 0; i < that.weaponArray.length; i++) {
        if (
          that.actor.property.canvasX + that.actor.property.characterWidth >=
            that.weaponArray[i].weapon.canvasX &&
          that.actor.property.canvasX + that.actor.property.characterWidth <=
            that.weaponArray[i].weapon.canvasX +
              that.weaponArray[i].weapon.characterWidth &&
          that.actor.property.canvasY + that.actor.property.characterHeight >=
            that.weaponArray[i].weapon.canvasY &&
          that.actor.property.canvasY + that.actor.property.characterHeight <=
            that.weaponArray[i].weapon.canvasY +
              that.weaponArray[i].weapon.characterHeight
        ) {
          that.ctx.drawImage(
            that.assets.getImage('swap'),
            0,
            0,
            50,
            50,
            that.actor.property.canvasX + 20,
            that.actor.property.canvasY - 30,
            30,
            30
          );
  ...
  this.mouseMoved = function(event) {
    ...
    var dx =
      event.clientX -
      (that.actor.property.canvasX + that.actor.handProperty.x) -
      offset.left;
    var dy =
      event.clientY -
      (that.actor.property.canvasY + that.actor.handProperty.y) +
      20;
